    ...
    if storm_cfg.get("enabled"):
        ...
        if scrape_cfg:
            ...
            try:
                ...
                LOGGER.warning(
                    "Storm report scrape failed (%s); attempting catalog fallback",
                    exc,
                )
                catalog = None
            else:
        ...
        if catalog is None:
            ...
            if catalog_path:
                ...
                LOGGER.warning(
                    "Storm filter disabled: no catalog available after scrape failure"
                )
                catalog = None
        ...
        if catalog is not None:
            ...
            selection = storm_filter.resolve_event_window(
                ...
                auto_backfill_step_days=int(
                    storm_cfg.get("auto_backfill_step_days", 1) or 1
                ),
